=== EndPoint Security – Firewall & Malware Scan & Blacklist removal ===
Contributors: EndPoint Security
Tags: security, firewall, malware scanner, web application firewall, country blocking, clean site, blocklist, waf, login security
...
Secure your website with the most powerful WordPress security plugin. Firewall (WAF), malware scan, blocking, backdoor detection, login security & more.
...
The EndPoint Security plugin will take your website security to a whole new level.
...
The EndPoint Security plugin doesn't slow down your site and it is 100% free.
...
EndPoint Security starts with a list of one-click hardening techniques that will instantly add layers of protection to your site.

EndPoint Security adds the best in WordPress security plugin to your website with just a few clicks. Stop brute force attacks, SQL injections, cross-site scripting XSS, and other WordPress vulnerabilities and hacks with EndPoint Security malware scans, antivirus scans, IP blocking, firewall, activity log, security log, and two-factor authentication login security.


### WORDPRESS SECURITY SCANS
Run free malware scans that check WordPress for suspicious code and malware. The EndPoint Security scan tool compares your WordPress install with the master copy in WP directory, reports changes and lets you restore the original file with a click.

### Firewall and IP Manager
Keep your site safe with EndPoint Security’s IP manager and firewall. Manually block specific IPs, import a list of banned IPs and set automated timed and permanent lockouts. EndPoint Security makes it easy to block and unblock specific locations quickly thanks to its advanced firewall (WAF).
...
EndPoint Security runs surveillance and sends notifications with information that matters.

### Login Protection

Brute force attacks are no match for EndPoint Security. Limit login attempts to stop users trying to guess passwords. Permanently ban IPs or trigger a timed lockout after a set number of failed login attempts.

### Login Screen Masking

EndPoint Security makes it easy to move your login screen to a custom URL. Not only does login screen masking improve security, but it also lets you white label your login user experience and improves branding. 
